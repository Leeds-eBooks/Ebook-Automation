...
var edit = {
  ...
  xhtml_Indexer: function(doc) {
    return doc.replace(/(<a\s+[^>]*id="page)("[^>]*>[^<]*<\/a>)/gi, function(m, a, b) {
        return a + (numberIndex++) + b;
    });
  },
  xhtml_UpperCase: function(doc) {
    return doc.replace(/<small>(.+?)<\/small>/gi, function(m, a) {
      var smallcaptext = a.toUpperCase();
        return '<small>' + smallcaptext + '</small>'
    });
  },
  ...
  css_Regexes: function(doc) {
   ...
   css_PxToEm: function(doc) {
    return doc
      .replace(/(\d+)px/g, function pxToEm(x) {
        x = x.substring(0, x.length - 2);
       return (x / 16).toFixed(2) + "em";
    });
  },
